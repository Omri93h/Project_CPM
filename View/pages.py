...
from .entry_component import EntryWithPlaceholder
...
class DashboardPage(Frame):
    def __init__(self, master, controller, params=None):
        ...
        label_total_str = Label(
            self, text="Total Clients Balance:", font='Ubuntu 14')
        label_total_str.pack(pady=(20, 10), fill=X)
        label_total_num = Label(
            self, text=str(controller.getTotalBalance()) + "$", font='Ubuntu 26 bold')
...
class EditBrokerPage(Frame):
    def __init__(self, master, controller):
        ...
        self.entry_API_KEY = Entry(broker_data_frame, width=65)
        self.entry_API_KEY.grid(row=0, column=1)
        self.entry_API_KEY.insert(0, controller.model.broker.API_KEY)

        Label(broker_data_frame, text="Binance API SECRET: ").grid(
            row=1, column=0)
        self.entry_API_SECRET = Entry(broker_data_frame, show="*", width=65)
        ...
        self.entry_username = Entry(broker_data_frame, width=65)
        ...
        self.entry_name = Entry(broker_data_frame, width=65)
        ...
        self.entry_password = Entry(broker_data_frame, show="*", width=65)
...
class EditClientPage(Frame):
    def __init__(self, master, params=None):
        ...
        Button(header_frame, text="Back",
               command=lambda: master.switchFrame(PortfolioPage, params)).grid(
                   row=0, column=0, ipadx=30
        )
...
class NewOrderPage(Frame):
    def __init__(self, master, params=None):
        ...
        label_header.grid(row=0, column=1, ipadx=30, padx=30)
        Button(header_frame, text="Save", bg="blue", fg="white",
               command=lambda: self.Save(params[0], params[1].id)).grid(
                   row=0, column=2, ipadx=30
        )
        ...
        self.entry_amount = EntryWithPlaceholder(self, "Enter Amount ...")
        self.entry_price = EntryWithPlaceholder(self, "Enter Price ...")
        ...
        Button(frame_order_action, text="BUY", bg="green", font="Ubuntu 18 bold", fg="white",
               command=lambda:  self.createOrder(params, "Buy")).grid(
            row=0, column=0, padx=20, pady=10)

        Button(frame_order_action, text="SELL", bg="red", font="Ubuntu 18 bold", fg="white",
    ...
    def createOrder(self, params, action):
        ...
        response = params[0].model.broker.createOrder(
            params[1].id, self.orderDetails)

        if response:
            self.responseText.set("Order Created Successfully !")
        else:
            self.responseText.set("Something went wrong with your order !\n "
                                  "Check internet connection, amount and price\n"
                                  "and try again")
...
class PortfolioPage(Frame):
    def __init__(self, master, params=None):
        ...
        Button(actions_frame, text="Edit Client Info", bg="white",
               command=lambda:
